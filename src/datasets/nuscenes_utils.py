...
def heading_change_rate(h, t):
    ...
    hnan = np.isnan(h).astype(np.int_)
    if np.sum(hnan) == 0:
        return hdot
    lead_nans = (hnan[1:] - hnan[:-1]) == -1
    lead_nans = np.append([False], lead_nans)
    repl_idx = np.append([False], lead_nans[:-1])
    num_fill = np.sum(repl_idx.astype(np.int_))
    if num_fill != 0:
        if num_fill != np.sum(lead_nans.astype(np.int_)):
...
def velocity(pos, t):
    ...
    # for any nan -> value transition frames, want to use forward difference
    posnan = np.isnan(np.sum(pos, axis=1)).astype(np.int_)
    if np.sum(posnan) == 0:
        return vel
...
def get_adv_coloring(NA, attack_agt, tgt_agt,
    ...
    if markersize:
        base_msize = [1.0]*NA
        if attack_agt is not None:
            base_msize[attack_agt] = 2.0
        if tgt_agt is not None:
            base_msize[tgt_agt] = 2.0
        ...
        if viz_traj:
            ...
            if len(car_kin.size()) == 3:
                ...
                for n in range(NA):
                    ...
                    # When visualizing video frames, we may pass a single-step trajectory (T=1).
                    # In that case, avoid indexing into a length-1 linspace with a boolean mask
                    # coming from torch (can trigger edge-case IndexError in some numpy versions).
                    if T <= 1:
                        carr = np.array([0.0], dtype=np.float32)[: cur_traj.size(0)]
                    else:
                        carr = np.linspace(0, 1.0, T)[val_steps.cpu().numpy()]
                    NT = cur_traj.size(0)
            ...
            if viz_init_car:
                # plot the car at the first valid frame (skip all-NaN agents)
                if len(car_kin.size()) == 3:
                    NA, _, _ = car_kin.size()
                    for n in range(NA):
                        valid_idx = torch.nonzero(~torch.isnan(car_kin[n, :, 0]), as_tuple=True)[0]
                        if valid_idx.numel() == 0:
                            continue
                        nonnan_idx = valid_idx[0]
                        cur_col = color[n] if color is not None else plt_color(n % 9)
                        cur_alph = alpha[n] if alpha is not None else 0.7
                    ...
                    for n in range(NA):
                        for s in range(1):
                            valid_idx = torch.nonzero(~torch.isnan(car_kin[n, s, :, 0]), as_tuple=True)[0]
                            if valid_idx.numel() == 0:
                                continue
                            nonnan_idx = valid_idx[0]
                            cur_col = color[n] if color is not None else plt_color(n % 9)
