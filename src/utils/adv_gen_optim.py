...
from losses.adv_gen_nusc import interp_traj, check_single_veh_coll_min_box_gap
...
def run_adv_gen_optim_advsim(cur_z, lr, loss_weights, model, scene_graph, map_env, map_idx,
                    num_iters, embed_info, planner_name, tgt_prior_distrib, other_prior_distrib,
                    feasibility_time, feasibility_infront_min,
                    planner=None, planner_viz_out=None,
                    attack_agt_idx=None,
    ...
    for oidx in pbar_optim:
        def closure():
            ...
            # forward pass to compute target-specific loss
            tgt_decoder_out = model.dyna_to_traj(scene_graph, tgt_loss_input_z.reshape(tgt_loss_input_z.size(0), -1, 2))

            # forward pass to compute controlled agent loss
            other_decoder_out = model.dyna_to_traj(scene_graph, other_loss_input_z.reshape(other_loss_input_z.size(0), -1, 2))
            
            # rollout planner allowing it to react to the current model rollout
            if planner_name == 'hardcode':
                cur_agt_pred = tgt_decoder_out[~ego_mask]
                cur_agt_pred = model.normalizer.unnormalize(cur_agt_pred).detach().cpu().numpy()
                planner_fut = planner.rollout(cur_agt_pred, plan_t, cur_agt_ptr.cpu().numpy(), plan_t,
                                                # viz=True,
                                                control_all=False).to(scene_graph.future_gt)
            ...
            # compute each loss
            loss_dict = dict()
            tgt_match_loss_dict = tgt_loss(model.get_normalizer().unnormalize(tgt_decoder_out[ego_mask]),
                                model.get_normalizer().unnormalize(planner_fut),
                                tgt_z,
                                tgt_prior_distrib)
            loss_dict = {'tgt_match_' + k : v for k, v in tgt_match_loss_dict.items()}

            tgt_traj = planner_fut if not adv_use_own_pred else other_decoder_out[ego_mask]
            adv_loss_dict = adv_loss(model.get_normalizer().unnormalize(other_decoder_out),
                                        model.get_normalizer().unnormalize(tgt_traj),
                                        other_z,
                                        other_prior_distrib,
                                        attack_agt_idx=attack_agt_idx)
    ...
    cur_z = collate_tgt_other_z(scene_graph, tgt_z, other_z_all)
    with torch.no_grad():
        final_decoder_out = model.dyna_to_traj(scene_graph, cur_z.reshape(cur_z.size(0), -1, 2))
    final_result_traj = final_decoder_out.unsqueeze(1).clone().detach()
    if planner_name == 'ego':
        # replace the ego (planner) traj with GT
        final_result_traj[ego_inds, torch.zeros_like(ego_inds)] = scene_graph.future_gt[ego_mask][:, :, :4]
    elif planner_name == 'hardcode':
        # replace ego (planner) traj with actual output of planner
        cur_agt_pred = final_decoder_out[~ego_mask]
        cur_agt_pred = model.normalizer.unnormalize(cur_agt_pred).detach().cpu().numpy()
        planner_fut = planner.rollout(cur_agt_pred, plan_t, cur_agt_ptr.cpu().numpy(), plan_t,
                                        viz=planner_viz_out,
                                        control_all=False).to(scene_graph.future_gt)
    ...
    # compute loss one more time to get final min agt/t
    tgt_traj = final_result_traj[ego_inds, torch.zeros_like(ego_inds)] # the true planner rollout
    adv_loss_dict = adv_loss(model.get_normalizer().unnormalize(final_decoder_out),
                                model.get_normalizer().unnormalize(tgt_traj),
                                cur_z[~ego_mask].clone().detach(),
                                other_prior_distrib,
                                return_mins=True)
...
def compute_adv_gen_success(final_result_traj, model, scene_graph, attack_agt):
    ...
    n_other = other_fut.size(0)
    if n_other <= 0:
        return False, None

    attack_other_idx = int(attack_agt) - 1

    veh_coll, coll_time = check_single_veh_coll_min_box_gap(
        planner_gt_fut,
        planner_lw,
        other_fut,
        other_lw
    )
    ft = planner_gt_fut.size(0)
    if not bool(veh_coll[attack_other_idx]):
        return False, None
    t = int(coll_time[attack_other_idx])
    if t >= ft:
        return False, None
    return True, t + 1