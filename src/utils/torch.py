...
def load_model_state(model, model_state_dict, ignore_keys=None):
    if ignore_keys is not None:
        model_state_dict = {k: v for k, v in model.items() if k.split('.')[0] not in ignore_keys}
...
def load_state(load_path, model, optimizer=None, map_location=None, ignore_keys=None, module_keys=None):
    ...
    model_state_dict = full_checkpoint_dict['model'] if module_keys is None else full_checkpoint_dict[module_keys[0]]
    optim_state_dict = full_checkpoint_dict['optim'] if module_keys is None else full_checkpoint_dict[module_keys[-1]]
