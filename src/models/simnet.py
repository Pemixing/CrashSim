...
class SimNet(nn.Module):
    def __init__(self, npast, nfuture, map_obs_size_pix, nclasses,
        ...
        #                                          self.z_size*2,
        #                                          )
        self.prior_net = MLP([self.past_feat_size + self.map_feat_out_size + self.NC, 128, 128, 128, self.z_size*2])
        self.posterior_net = MLP([self.future_feat_size + self.past_feat_size + self.map_feat_out_size + self.NC, 128, 128, 128, self.z_size*2])
        ...
        #                                         self.traj_out_size
        #                                         )
        self.decoder_net = MLP([decode_in_size, 128, 128, 64, self.traj_out_size])
    ...
    def make_samples(self, scene_graph, map_idx, map_env, test_sample_num):
        sample_pred = self.sample_batched(scene_graph, map_idx, map_env, test_sample_num,
                                    include_mean=False)
        return sample_pred['future_pred']
    ...
    def encoder(self, scene_graph, map_feat, past_feat, future_feat):
        ...
        posterior_z = self.posterior_net(posterior_in_feat)
    ...
    def prior(self, scene_graph, map_feat, past_feat):
        ...
        prior_z = self.prior_net(prior_in_feat)
    ...
    def autoregressive_decoder(self, scene_graph, map_feat, past_feat, z, map_idx, map_env,
        ...
        for t in range(FT):
            ...
            decoder_out = self.decoder_net(decoder_in_feat) # (NA, traj_dim)
