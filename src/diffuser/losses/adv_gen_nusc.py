    ...
    def forward(self, future_pred, tgt_traj, z, prior_out,
        ...
        if 'adv_crash' in self.loss_weights and self.loss_weights['adv_crash'] > 0.0:
            ...
            if attack_agt_idx is not None:
                attack_agt_mask = torch.zeros(future_pred.size(0), dtype=torch.bool)
                attack_agt_mask[attack_agt_idx] = True
                attack_agt_mask = attack_agt_mask[~self.ego_mask].unsqueeze(1).expand_as(min_dist_in)
                attack_agt_mask = attack_agt_mask.to(min_dist_in.device)
...
def check_single_veh_coll(traj_tgt, lw_tgt, traj_others, lw_others):
    ...
    coll_time = np.ones((NA), dtype=np.int_)*FT
    ...
    for aj in range(NA):
        for t in range(FT):
            ...
            if t not in poly_cache:
                ...
                if not is_valid_polygon(ai_corners):
                    break
...
def is_valid_polygon(corners):
    ...
    if not np.isfinite(corners).all():
        return False
    if np.allclose(corners, corners[0]):  # all points same
        return False
    return True

# Minimum oriented-box boundary distance (meters); <= gap counts as collision for min-gap metric.
VEH_COLL_MIN_GAP_THRESH = 0.1


def check_single_veh_coll_min_box_gap(traj_tgt, lw_tgt, traj_others, lw_others, gap_thresh=None):
    '''
    Planner vs others: collision when the minimum Euclidean distance between oriented
    vehicle rectangles (nuScenes-style corners from get_corners) is <= gap_thresh.

    Unlike check_single_veh_coll (IoU ratio), this uses pure geometric clearance between
    box boundaries; overlapping or touching boxes yield distance 0.

    Assumes UNNORMALIZED trajectories and lw. Skips other-agent timesteps with NaNs.

    :param traj_tgt: (T x 4)
    :param lw_tgt: (2, )
    :param traj_others: (N x T x 4)
    :param lw_others: (N x 2)
    :param gap_thresh: if None, uses VEH_COLL_MIN_GAP_THRESH

    :returns veh_coll: (N,) bool
    :returns coll_time: (N,) int first timestep with collision
    '''
    import datasets.nuscenes_utils as nutils
    from shapely.geometry import Polygon

    if gap_thresh is None:
        gap_thresh = VEH_COLL_MIN_GAP_THRESH
    ...
    for aj in range(NA):
        for t in range(FT):
            if t not in tgt_poly_cache:
                ai_state = traj_tgt[t, :]
                ai_corners = nutils.get_corners(ai_state, lw_tgt)
                if not is_valid_polygon(ai_corners):
                    break
                tgt_poly_cache[t] = Polygon(ai_corners)
            ai_poly = tgt_poly_cache[t]

            aj_state = traj_others[aj, t, :]
            if np.sum(np.isnan(aj_state)) > 0:
                continue
            aj_corners = nutils.get_corners(aj_state, lw_others[aj])
            if not is_valid_polygon(aj_corners):
                continue
            aj_poly = Polygon(aj_corners)
            min_gap = ai_poly.distance(aj_poly)
            if min_gap <= gap_thresh + 1e-9:
                veh_coll[aj] = True
                coll_time[aj] = t
                break
